...
class Synth extends React.Component<{}, State> {
  constructor(props: {}) {
    ...
    this.state = {
      ...
      pressedKeys: [],
      keys,
      oscillatorConfigs: [oscillatorConfig],
      filter: {
        type: "lowpass",
        frequency: 1000,
      },
  ...
  componentWillMount() {
    ...
    const oscillatorConfigs = this.state.oscillatorConfigs.map((o, id) => ({
      ...o,
      id,
    }));
  ...
  setFilterFrequency = (frequency: number) => {
    ...
    this.setState({
      oscillatorConfigs: [
        ...this.state.oscillatorConfigs.map((x) => {
          if (x.filter) {
            x.filter.frequency = frequency;
          }
          return x;
        }),
      ],
      filter: { ...this.state.filter, frequency },
    });
  };
  ...
  playNote = (key: KeyType) => {
    if (
      keyExists(key.keyPress) &&
      !keyIsPressed(this.state.pressedKeys, key.keyPress)
    ) {
  ...
  stopNote = (keyPress: string) => {
    if (keyExists(keyPress)) {
      const oscillators = this.state.synth.oscillators.filter(
        (o) => o.keyPress === keyPress
      );
      ...
      this.setState({
        ...
        synth: {
          ...this.state.synth,
          oscillators: this.state.synth.oscillators.filter(
            (o) => o.keyPress !== keyPress
          ),
        },
      });
